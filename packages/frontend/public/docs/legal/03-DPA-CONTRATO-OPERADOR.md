# DATA PROCESSING AGREEMENT (DPA)
## Contrato de Tratamento de Dados Pessoais

**Versão:** 1.0
**Última atualização:** 04/05/2026

---

## QUALIFICAÇÃO DAS PARTES

### CONTROLADOR
**Razão Social:** ____________________________________________
**CNPJ:** _______________________
**Endereço:** ____________________________________________
**Representante Legal:** ____________________________________________
**CPF:** _______________________
**E-mail:** _______________________
**Telefone:** _______________________
**Encarregado de Dados (DPO) do Cliente:** ____________________________________________
**E-mail do DPO:** _______________________

### OPERADOR
**Razão Social:** KONTRATA.AI
**CNPJ:** [a preencher]
**Endereço:** [endereço completo]
**Representante Legal:** Roberto [completar]
**CPF:** [a preencher]
**E-mail:** kontrataai@gmail.com
**DPO:** kontrataai@gmail.com

---

## CONSIDERANDOS

CONSIDERANDO que o Controlador firmou Contrato Principal de Prestação de Serviços com o Operador para uso da plataforma **Kontrata.ai**;

CONSIDERANDO que o uso da plataforma envolve tratamento de **dados pessoais** de Titulares (colaboradores, candidatos, clientes finais, fornecedores) sob responsabilidade do Controlador;

CONSIDERANDO que a **Lei nº 13.709/2018 — Lei Geral de Proteção de Dados Pessoais (LGPD)** exige formalização das responsabilidades entre Controlador e Operador (Art. 5, VII e VIII; Art. 39);

CONSIDERANDO o disposto também no **Marco Civil da Internet (Lei nº 12.965/2014)**, **Código Civil**, **CLT** e demais normas aplicáveis;

As partes celebram o presente **CONTRATO DE TRATAMENTO DE DADOS PESSOAIS** ("DPA"), que se rege pelas seguintes cláusulas:

---

## CLÁUSULA 1ª — OBJETO

1.1. Este DPA regula as condições, finalidades, garantias e responsabilidades das partes em relação ao tratamento de dados pessoais decorrente da execução do Contrato Principal de Prestação de Serviços da plataforma Kontrata.ai.

1.2. Este DPA integra e complementa o Contrato Principal e os Termos de Uso, prevalecendo, em caso de conflito, sobre cláusulas contraditórias relativas a tratamento de dados pessoais.

---

## CLÁUSULA 2ª — DEFINIÇÕES

Aplicam-se as definições da LGPD (Art. 5), em especial:

- **Dado Pessoal**: informação relacionada a pessoa natural identificada ou identificável.
- **Dado Pessoal Sensível**: dado sobre origem racial ou étnica, convicção religiosa, opinião política, filiação a sindicato ou a organização de caráter religioso/filosófico/político, dado referente à saúde ou à vida sexual, dado genético ou biométrico.
- **Tratamento**: toda operação realizada com dados pessoais (coleta, uso, armazenamento, eliminação, etc.).
- **Titular**: pessoa natural a quem se referem os dados pessoais.
- **Controlador**: a quem competem as decisões sobre o tratamento.
- **Operador**: quem realiza o tratamento em nome do Controlador.
- **Encarregado / DPO**: pessoa indicada pelo Controlador e Operador como canal de comunicação.
- **Incidente de Segurança**: evento adverso que afete dados pessoais (vazamento, acesso não autorizado, alteração indevida, perda).
- **Sub-operador**: terceiro contratado pelo Operador para auxiliar no tratamento.

---

## CLÁUSULA 3ª — PAPÉIS

3.1. **Para todos os fins legais**:
   - O **CLIENTE** atua como **CONTROLADOR** dos dados pessoais inseridos na plataforma, sendo responsável pela legalidade da coleta, finalidade, base legal, retenção e atendimento dos direitos dos Titulares;
   - A **Kontrata.ai** atua como **OPERADOR**, processando os dados estritamente conforme instruções documentadas do Controlador.

3.2. **A Kontrata.ai atuará como Controlador apenas em relação a**:
   - Dados de cadastro do próprio Cliente;
   - Dados de Usuários administradores;
   - Dados de cobrança e faturamento;
   - Logs técnicos e de auditoria gerados pela operação da Plataforma;
   - Dados estatísticos agregados e anonimizados.

3.3. As instruções do Controlador são consideradas dadas pelo Contrato Principal, Termos de Uso, este DPA, e pelos atos praticados pelos Usuários autorizados na Plataforma. Instruções adicionais devem ser formalizadas por escrito.

---

## CLÁUSULA 4ª — FINALIDADES E NATUREZA DO TRATAMENTO

4.1. **Finalidades**: gestão de RH, prevenção de perdas, gestão comercial, recrutamento e seleção, acompanhamento operacional e demais finalidades dos módulos contratados.

4.2. **Natureza do tratamento**: coleta, registro, organização, armazenamento, consulta, transmissão, modificação, exclusão.

4.3. **Categorias de Titulares**: colaboradores ativos e desligados, candidatos, dependentes, clientes finais (quando aplicável), fornecedores, visitantes (quando aplicável).

4.4. **Categorias de dados** (resumo — detalhamento na Política de Privacidade e ROPA):
   - Identificação: nome, CPF, RG, CTPS, PIS;
   - Contato: e-mail, telefone, endereço;
   - Profissionais: cargo, salário, jornada;
   - Bancários;
   - Sensíveis: saúde (ASOs, atestados), biometria (Vision Facial), origem racial/étnica (eSocial), filiação sindical (descontos);
   - Imagens (foto, gravações de câmera);
   - Comportamentais (DISC, performance);
   - Histórico (bipagens, presenças, ocorrências).

4.5. **Duração do tratamento**: durante a vigência do Contrato Principal e prazos de retenção legais aplicáveis pós-término.

---

## CLÁUSULA 5ª — OBRIGAÇÕES DO CONTROLADOR

5.1. O Controlador obriga-se a:

a) **Assegurar base legal adequada** para cada operação de tratamento (consentimento, execução de contrato, obrigação legal, legítimo interesse, proteção da vida, etc., conforme Arts. 7 e 11 da LGPD);

b) **Cumprir os princípios da LGPD** (Art. 6): finalidade, adequação, necessidade, livre acesso, qualidade, transparência, segurança, prevenção, não discriminação, responsabilização e prestação de contas;

c) **Informar adequadamente os Titulares** sobre o tratamento, fornecendo Política de Privacidade própria, avisos de coleta, e quando exigido, **obter consentimento específico**;

d) **Não inserir dados de Titulares sem vínculo justificável**, ou para finalidades incompatíveis com a coleta original;

e) **Obter consentimento específico para dados sensíveis** quando esta for a base legal aplicável, em especial:
   - Imagem facial para Vision Facial (assinatura do Termo de Consentimento Biométrico);
   - Currículos de candidatos (aceite no formulário público);
   - Dados de saúde além do estritamente exigido pela legislação trabalhista;

f) **Atender as solicitações dos Titulares** quanto aos seus direitos (Art. 18 LGPD), no prazo de 15 dias, valendo-se das funcionalidades da Plataforma para acesso, correção, exportação e exclusão;

g) **Configurar adequadamente** as permissões de acesso de seus Usuários, aplicando o princípio do menor privilégio;

h) **Não compartilhar credenciais** de acesso e revogar imediatamente acessos de pessoas que se desliguem;

i) **Notificar a Kontrata.ai em até 24 horas** sobre qualquer incidente, suspeita de incidente, ou requisição da ANPD ou de Titulares relacionada à Plataforma;

j) **Indenizar a Kontrata.ai** por demandas decorrentes do descumprimento das obrigações desta cláusula 5;

k) **Manter Encarregado (DPO) próprio** ou autorizar formalmente a Kontrata.ai a indicar um, conforme suas necessidades;

l) **Responder, perante os Titulares e a ANPD**, pelas decisões de tratamento que tomar;

m) **Cumprir as legislações setoriais aplicáveis**: CLT, eSocial, NRs, Regulamentos sanitários, regras tributárias, Marco Civil da Internet, e demais.

---

## CLÁUSULA 6ª — OBRIGAÇÕES DO OPERADOR

6.1. A Kontrata.ai, como Operador, obriga-se a:

a) **Tratar os dados estritamente conforme instruções** documentadas do Controlador, salvo se exigido por lei;

b) **Notificar o Controlador imediatamente** caso receba instrução que entenda ser ilegal ou em violação à LGPD, sem cumpri-la até esclarecimento;

c) **Implementar e manter medidas técnicas e administrativas** adequadas e proporcionais ao risco do tratamento, incluindo (mas não limitado a):
   - Criptografia em trânsito (HTTPS/TLS) em todas as interfaces;
   - Criptografia em repouso para dados sensíveis (em implementação);
   - Hashing de senhas com algoritmo robusto (bcrypt);
   - Isolamento lógico de dados por Cliente (multi-tenant com bancos separados);
   - Controles de acesso baseados em perfil (RBAC);
   - Logs de auditoria de acesso a dados sensíveis;
   - Backups automatizados diários com retenção de 30 dias;
   - Proteção contra DDoS, SQL Injection, XSS, CSRF;
   - Atualização periódica de dependências de software;
   - Acordos de confidencialidade com colaboradores;

d) **Garantir confidencialidade** dos dados acessados, mesmo após o término do contrato, por todo o pessoal envolvido;

e) **Não utilizar os dados para finalidade diversa** da execução do contrato;

f) **Não compartilhar os dados** com terceiros, exceto sub-operadores autorizados ou por exigência legal;

g) **Auxiliar o Controlador** no atendimento das requisições de Titulares (no prazo solicitado, observado o limite legal);

h) **Auxiliar o Controlador** no cumprimento dos Arts. 32 a 38 da LGPD (relatório de impacto, segurança, comunicação à ANPD, etc.) na medida do tecnicamente possível;

i) **Notificar o Controlador em até 24 (vinte e quatro) horas** após tomar conhecimento de incidente de segurança que envolva dados do Controlador;

j) **Manter ROPA** (Registro de Operações de Tratamento) — Art. 37 LGPD;

k) **Designar e manter Encarregado de Dados (DPO)** acessível;

l) **Devolver ou eliminar os dados pessoais** ao término do contrato, conforme Cláusula 12;

m) **Disponibilizar ao Controlador**, mediante solicitação razoável, informações suficientes para demonstrar o cumprimento desta DPA;

n) **Aceitar auditorias** do Controlador ou de terceiro por ele indicado, mediante aviso prévio mínimo de 30 dias e em horário comercial, com critérios e escopo previamente acordados.

---

## CLÁUSULA 7ª — SUB-OPERADORES

7.1. O Controlador autoriza, de modo geral, a contratação dos seguintes sub-operadores:

| Sub-operador | Finalidade | País de Tratamento |
|--------------|------------|---------------------|
| Hostinger International Ltd | Hospedagem em nuvem (VPS) | Brasil / Lituânia |
| Cloudflare Inc. | CDN, DNS, proteção DDoS | Global |
| (provedor de e-mail SMTP a definir) | Envio de e-mails transacionais | EUA |
| Anthropic PBC | IA conversacional (Recrutador IA, quando aplicável) | EUA |
| OpenAI LLC | IA conversacional (modelos alternativos, quando aplicável) | EUA |
| Meta Platforms Inc. | WhatsApp Business API (quando aplicável) | Global |

7.2. A Kontrata.ai poderá **alterar a lista de sub-operadores** mediante:
   a) Aviso prévio de 30 dias ao Controlador;
   b) Direito do Controlador de objetar fundamentadamente, com possibilidade de rescisão do contrato sem ônus caso a alternativa não seja viável.

7.3. A Kontrata.ai **garante que cada sub-operador** está vinculado a obrigações de proteção de dados **equivalentes ou superiores** a este DPA, mediante contratos específicos.

7.4. **Responsabilidade**: A Kontrata.ai **permanece responsável** perante o Controlador pelo cumprimento das obrigações pelos sub-operadores.

---

## CLÁUSULA 8ª — TRANSFERÊNCIA INTERNACIONAL

8.1. Pode haver transferência internacional de dados aos sub-operadores listados, especialmente para CDN e provedores de IA com infraestrutura nos EUA.

8.2. A Kontrata.ai compromete-se a adotar, para cada transferência:
   a) Cláusulas contratuais padrão equivalentes às SCCs europeias adaptadas à LGPD;
   b) Avaliação de adequação do país (quando publicada lista pela ANPD);
   c) Medidas técnicas suplementares (criptografia, pseudonimização) quando aplicável.

8.3. A Kontrata.ai fornecerá, mediante solicitação razoável, informações sobre as garantias adotadas.

---

## CLÁUSULA 9ª — INCIDENTES DE SEGURANÇA

9.1. **Notificação ao Controlador**: A Kontrata.ai notificará o Controlador em até **24 (vinte e quatro) horas** após tomar conhecimento de incidente que afete dados pessoais sob responsabilidade do Controlador, contendo:
   a) Descrição do incidente e dados envolvidos;
   b) Categorias e número aproximado de Titulares afetados;
   c) Causa provável;
   d) Medidas tomadas e a tomar;
   e) Possíveis riscos e impactos.

9.2. **Notificação à ANPD e Titulares**: É **responsabilidade do Controlador** notificar a ANPD e os Titulares afetados quando legalmente exigido. A Kontrata.ai prestará todo apoio técnico necessário.

9.3. **Cooperação**: As partes cooperarão de forma diligente, transparente e tempestiva na resposta ao incidente, mitigação de danos e investigação.

9.4. **Documentação**: A Kontrata.ai manterá registro detalhado do incidente e medidas adotadas, disponível ao Controlador e à ANPD se solicitado.

---

## CLÁUSULA 10ª — DIREITOS DOS TITULARES

10.1. A Kontrata.ai disponibilizará funcionalidades técnicas para que o Controlador atenda os direitos dos Titulares:
   a) Acesso aos dados (tela "Meus Dados");
   b) Correção (edição direta);
   c) Exportação (download de dados em formato estruturado);
   d) Exclusão / Anonimização (botão de exclusão com retenção legal observada);
   e) Logs de quem acessou determinado dado (para investigação).

10.2. **Solicitações recebidas pela Kontrata.ai diretamente**: serão **encaminhadas ao Controlador em até 5 dias úteis** para que ele as atenda, salvo se o Titular for diretamente Cliente, Usuário ou Visitante (caso em que a Kontrata.ai atende como Controlador).

10.3. **Auxílio técnico**: A Kontrata.ai prestará auxílio razoável e proporcional, podendo cobrar custos quando a solicitação for excessiva ou repetitiva, conforme legislação.

---

## CLÁUSULA 11ª — AUDITORIA E COMPROVAÇÃO

11.1. A Kontrata.ai fornecerá ao Controlador, mediante solicitação razoável:
   a) Cópia da Política de Privacidade e desta DPA;
   b) Lista atualizada de sub-operadores;
   c) Resumo das medidas de segurança adotadas;
   d) Relatório de incidentes (quando aplicável);
   e) Certificações ou pareceres técnicos (quando disponíveis).

11.2. **Auditoria presencial ou remota**: O Controlador pode auditar o cumprimento desta DPA, mediante:
   a) Aviso prévio de 30 dias;
   b) Limitação a horário comercial;
   c) Limitação de frequência (uma vez por ano, exceto em caso de incidente grave);
   d) Cobertura por NDA;
   e) Custos arcados pelo Controlador (incluindo eventual reembolso do tempo da equipe da Kontrata.ai, exceto quando a auditoria identificar não conformidade relevante);
   f) Escopo previamente acordado.

11.3. A Kontrata.ai poderá, em substituição a auditoria presencial, apresentar **relatório de auditor independente** (ex: SOC 2, ISO 27001, ISO 27701, quando aplicável).

---

## CLÁUSULA 12ª — TÉRMINO E DEVOLUÇÃO/EXCLUSÃO

12.1. **Após o término do Contrato Principal** (por qualquer motivo):

a) **Período de exportação**: O Controlador terá **30 (trinta) dias** a partir do término para exportar todos os seus dados via funcionalidade nativa da Plataforma;

b) **Exclusão**: Findo o prazo, a Kontrata.ai **excluirá ou anonimizará** todos os dados pessoais do Controlador, exceto:
   - Dados de cobrança e fiscais (5 anos para prescrição tributária);
   - Logs de auditoria mínimos exigidos pelo Marco Civil (6 meses);
   - Dados em backups, que serão purgados no ciclo natural (até 30 dias);

c) **Declaração de exclusão**: A Kontrata.ai fornecerá declaração formal ao Controlador confirmando a exclusão.

12.2. **Backup do Controlador**: O Controlador é responsável por extrair e armazenar suas próprias cópias antes do término. Após excluído, **a Kontrata.ai não pode recuperar os dados**.

12.3. **Continuidade de obrigações**: As obrigações de confidencialidade, segurança e cooperação em incidentes **sobrevivem** ao término do contrato pelos prazos legais.

---

## CLÁUSULA 13ª — RESPONSABILIDADES E LIMITAÇÕES

13.1. **Cada parte responde** pelos atos que praticar com violação à LGPD ou a este DPA, conforme Arts. 42 a 45.

13.2. **Solidariedade**: Reconhecem que, em razão do Art. 42, §1º, da LGPD, **podem responder solidariamente** perante Titulares e ANPD por danos decorrentes do tratamento. Sem prejuízo do direito de regresso.

13.3. **Limitação de responsabilidade da Kontrata.ai**: A responsabilidade total da Kontrata.ai perante o Controlador, em decorrência deste DPA, fica limitada conforme cláusula equivalente do Contrato Principal, salvo:
   a) Casos de dolo ou culpa grave;
   b) Multas administrativas aplicadas pela ANPD em razão exclusiva de ato/omissão da Kontrata.ai;
   c) Indenizações a Titulares por incidentes causados exclusivamente pela Kontrata.ai.

13.4. **Indenização cruzada**:
   a) A Kontrata.ai indenizará o Controlador por danos comprovadamente decorrentes de descumprimento das obrigações de Operador (Cláusula 6);
   b) O Controlador indenizará a Kontrata.ai por danos comprovadamente decorrentes de descumprimento das obrigações de Controlador (Cláusula 5).

13.5. **Notificação prévia**: Antes de qualquer demanda, a parte afetada deve notificar a outra com prazo razoável (mínimo 15 dias) para tentativa de mitigação.

---

## CLÁUSULA 14ª — DISPOSIÇÕES GERAIS

14.1. **Vigência**: Este DPA vigora pelo período do Contrato Principal e prazos legais aplicáveis após o término.

14.2. **Atualizações**: Atualizações exigidas por lei ou regulamentação serão incorporadas automaticamente. Demais alterações requerem aditivo.

14.3. **Independência**: Nulidade de cláusula não afeta as demais.

14.4. **Conflito de cláusulas**: Em caso de conflito entre este DPA e o Contrato Principal/Termos de Uso, **prevalece este DPA** quanto a tratamento de dados pessoais.

14.5. **Cessão**: Vide Contrato Principal.

14.6. **Notificações formais**: Por e-mail aos endereços qualificados, com confirmação de recebimento.

---

## CLÁUSULA 15ª — FORO

Aplica-se o foro do Contrato Principal. As partes comprometem-se a tentativa de solução amigável prévia.

---

## ASSINATURAS

E por estarem assim justas e contratadas, as partes assinam este instrumento em 2 (duas) vias de igual teor.

**Local:** ____________________________________
**Data:** ___/___/______

---

**CONTROLADOR**

____________________________________________
[Razão Social]
[Nome do Representante]
CPF: _______________________

---

**OPERADOR — KONTRATA.AI LTDA**

____________________________________________
[Nome do Representante]
CPF: _______________________

---

**TESTEMUNHAS:**

1) ____________________________________________
   Nome:
   CPF:

2) ____________________________________________
   Nome:
   CPF:

---

**Documento sujeito a revisão jurídica antes do uso comercial.**
