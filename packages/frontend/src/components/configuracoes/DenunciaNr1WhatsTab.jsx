...
export default function DenunciaNr1WhatsTab() {
  ...
  const carregar = async () => {
    try {
      ...
      setGroupId(cfg.whatsapp_group_denuncia_nr1 || '');
      setGroupName(cfg.whatsapp_group_denuncia_nr1_name || '');
    } catch { /* ignore */ }
  };

  const carregarPreview = async () => {
    try {
      const { data } = await api.get('/whatsapp/denuncia-nr1/preview');
  ...
  const salvar = async () => {
    ...
    try {
      await api.post('/config/configurations', {
        whatsapp_group_denuncia_nr1: groupId,
        whatsapp_group_denuncia_nr1_name: groupName,
      });
      flash('Configuração salva! Toda nova denúncia será notificada automaticamente nesse grupo.');
    } catch (e) {
  ...
  const testar = async () => {
    ...
    setEnviando(true);
    try {
      const { data } = await api.post('/whatsapp/denuncia-nr1/enviar');
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-gray-800">Denúncia NR-1</h2>
      </div>
      <p className="text-sm text-gray-500">
        <strong>Toda vez que uma denúncia for recebida</strong> no Canal de Denúncia (NR-1), o RH recebe automaticamente
        uma mensagem nesse grupo de WhatsApp com o <strong>nº do protocolo</strong> e um <strong>PDF</strong> com os detalhes,
        pra fazer a apuração. 🔒 Conteúdo confidencial — use um grupo restrito ao RH.
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input value={groupName} onChange={e => setGroupName(e.target.value)} placeholder="Ex: RH — Denúncias (restrito)"
        ...
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-2">
            ⚠️ O <strong>"Testar Envio"</strong> manda uma denúncia <strong>fictícia</strong> agora mesmo pro grupo (mensagem + PDF), só pra você ver como fica.
          </p>
        </div>
      </div>
    </div>
