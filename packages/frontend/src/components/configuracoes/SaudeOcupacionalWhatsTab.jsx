...
export default function SaudeOcupacionalWhatsTab() {
  const [groupId, setGroupId] = useState('');
  const [groupName, setGroupName] = useState('');
  const [diaSemana, setDiaSemana] = useState('1');
  const [horario, setHorario] = useState('08:00');
  const [antecedencia, setAntecedencia] = useState('45');
  ...
  const carregar = async () => {
    ...
    try {
      ...
      setGroupId(cfg.whatsapp_group_aso || '');
      setGroupName(cfg.whatsapp_group_aso_name || '');
      setDiaSemana(cfg.whatsapp_aso_dia_semana || '1');
      setHorario(cfg.whatsapp_aso_schedule_time || '08:00');
      ant = cfg.whatsapp_aso_dias_antecedencia || '45';
      setAntecedencia(ant);
    } catch { /* ignore */ }
    carregarPreview(ant);
  };

  const carregarPreview = async (ant) => {
    try {
      const a = ant ?? antecedencia;
      const { data } = await api.get(`/whatsapp/aso/preview?antecedencia=${a}`);
  ...
  const salvar = async () => {
    ...
    try {
      await api.post('/config/configurations', {
        whatsapp_group_aso: groupId,
        whatsapp_group_aso_name: groupName,
        whatsapp_aso_dia_semana: diaSemana,
        whatsapp_aso_schedule_time: horario,
        whatsapp_aso_dias_antecedencia: String(antecedencia || '45'),
      });
  ...
  const testar = async () => {
    ...
    setEnviando(true);
    try {
      const { data } = await api.post('/whatsapp/aso/enviar');
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-gray-800">Saúde Ocupacional (ASO)</h2>
      </div>
      <p className="text-sm text-gray-500">
        Envia automaticamente os ASOs <strong>vencidos</strong> e os que estão <strong>a vencer</strong> (com PDF detalhado
        por loja: colaborador, cargo e data de vencimento) para um grupo de WhatsApp, no dia e horário que você escolher.
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">🟡 Avisar a partir de</label>
              <div className="flex items-center gap-1">
                <input type="number" min="1" max="365" value={antecedencia}
                  onChange={e => setAntecedencia(e.target.value)}
                  onBlur={() => carregarPreview()}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500" />
          ...
          <p className="text-[11px] text-gray-500 -mt-2">
            ASOs que vencem dentro desse prazo entram na lista de <strong>"A Vencer"</strong> (ex: 45 = avisa 45 dias antes de vencer).
          </p>
        ...
        <div>
          ...
          <div className="bg-[#e5ddd5] rounded-xl p-3 min-h-[200px]">
            <div className="bg-white rounded-lg p-3 shadow-sm text-sm text-gray-800 whitespace-pre-wrap font-mono">
              {preview || 'Nenhum ASO vencido ou a vencer no momento.'}
            </div>
          </div>
          <p className="text-[11px] text-gray-500 mt-2">
